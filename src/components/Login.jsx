...
export default function Login() {
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    setLoading(true);
    setTimeout(() => { setLoading(false); navigate("/app/dashboard"); }, 1200);
  };
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 w-full max-w-sm px-8 py-10">
        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-4">
          ...
          <div className="flex justify-end">
            <Link to="/forgot-password" className="text-xs text-teal-500 hover:underline font-medium">
              Forgot password?
            </Link>
          </div>
        ...
        <p className="text-xs text-gray-400 text-center mt-5 leading-relaxed">
          ...
          <span className="text-teal-500 cursor-pointer hover:underline">Privacy Policy</span>{" "}and{" "}
        ...
        <div className="mt-6 pt-5 border-t border-gray-100 text-center">
          <p className="text-sm text-gray-500">
            ...
            <Link to="/signup" className="text-teal-500 font-semibold hover:underline">Sign up free</Link>
          </p>
        </div>
      </div>
    </div>
